...
class TestCommentsDAO:
    ...
    def test_get_by_post_pk_check_type(self, comments_dao):
        ...
        assert type(comments[0]) == dict, "Каждая позиция результата должна быть словарем"
    ...
    def test_get_by_post_pk_check_keys(self, comments_dao, keys_expected):
        ...
        assert comment_keys == keys_expected, "Список ключей не соответтвует"
    ...
    def test_get_by_post_pk_check_match(self, comments_dao, post_pk, correct_comments_pks):
        ...
        assert comment_pks == correct_comments_pks, f"Не слвпадают pk комментариев для поста {post_pk}"
